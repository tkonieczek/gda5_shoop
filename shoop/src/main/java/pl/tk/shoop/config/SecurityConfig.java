...
/*    @Override
    protected void configure(HttpSecurity http) throws Exception {
        http.formLogin().loginPage("/login")
                .and().rememberMe().tokenValiditySeconds(60).key("sdajava")
                .and()
                .authorizeRequests()
                .antMatchers(HttpMethod.GET, "/products/list").authenticated()
                .antMatchers(HttpMethod.POST, "/admin").hasAuthority("ADMIN")
                .antMatchers(HttpMethod.GET, "/admin/add").authenticated()
                .antMatchers(HttpMethod.GET, "/cart").authenticated()
                .antMatchers(HttpMethod.GET, "/cart/add").authenticated()
                .antMatchers(HttpMethod.POST, "/cart/recalculate").authenticated()
                .anyRequest().permitAll();
    }*/
    protected void configure(HttpSecurity http) throws Exception {
        http.formLogin().loginPage("/login")
                .and().rememberMe().tokenValiditySeconds(60).key("sdajava").and()
                .authorizeRequests()
                .antMatchers(HttpMethod.GET, "/products/list").authenticated()
                .antMatchers(HttpMethod.POST, "/admin").hasAuthority("ADMIN")
                .antMatchers(HttpMethod.GET, "/admin/add").authenticated()
